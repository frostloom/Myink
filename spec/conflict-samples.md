...
- **检测现状**：✅ **已落地（2026-08-12，L1 事件层近邻）**——当前章事件候选摘要 vs 历史事件向量近邻（cosine distance < 0.3 且章距 ≥ 10 → `hint`/style/local，每章至多 1 条，draft 含呼应标记词则整章豁免）；见 [l1.py](../src/aiink/validation/l1.py) `bridge_repeat_check` / [test_bridge_repeat.py](../tests/test_bridge_repeat.py)。**L2「呼应 vs 重复」判定属阶段 3 收尾**。
...
- **检测现状**：✅ **已落地（2026-08-12）**——draft/候选摘要含呼应标记词（当年/何其相似/重演/再现…）即豁免整章/该候选，实测 0 误报（即便 cosine distance=0.0 命中也豁免）；见 [l1.py](../src/aiink/validation/l1.py) `_has_callback_marker` / [test_bridge_repeat.py](../tests/test_bridge_repeat.py)。**词表未命中但实为呼应 → L2 抽样判定（后续）**。
...
1. **跑测**：阳性（样例 1–23、33–34）埋入章节样本或独立跑样例，跑 L1 + L2，统计**检出率**；阴性（样例 24–32、35）单独跑正常章节，统计**误报率**（阴性被检出 = 误报 +1）。两组缺一，指标不成立；
2. **调阈值**：L1 阈值宁缺毋滥（plan.md §8.8），先保证阴性 0 误报，再抬阳性检出率；L2 低置信度标 hint 不耗修订预算；
3. **长线**：样例 18/19/23/26/27 债务检查已落地为 **L1 每章 hint**（确定性、零模型成本、不阻塞，见 l1.py / test_debt_checks.py），与周期审计口径一致可直接对账；**样例 14/32 桥段重复的 L1 部分也已并入每章检查**（事件向量近邻 + 呼应词豁免，hint 不阻塞，见 l1.py `bridge_repeat_check` / test_bridge_repeat.py）；其余样例 11–13、15、31 及桥段重复的 L2 呼应判定走周期审计（每 K 章一次），不并入每章点检——避免成本与误报不可控；
