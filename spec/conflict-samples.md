# Ai Ink 冲突样例集 — Phase 0（32 例）

> **用途**：评测集（plan.md §16）。点级 10 例测 L1/L2 点校验（plan.md §8.4），长线级 5 例测长线一致性治理（plan.md §8.6），§7.7-7.9 记忆/图谱/伏笔线 12 例（样例 16-27）测人物状态台账 / 关系动态 / 伏笔 / 剧情线的一致性（§19.1 项 2），点/长线阴性对照 5 例（样例 28-32）测误报边界。任何优化跑同一套样例 + 同一 seed / prompt 模板 / 模型，用**检出率 / 误报率**度量，不挑成功案例。每例标注**检测现状**：已落地（代码可检出，见 test_flow.py）/ 待落地（机制已建模或已规划，阶段 3 长线治理主体落地）。新增样例统一标注 **阳性（应检出）/ 阴性（应不检出，误报控制）**。
>
...
> 8 例阳性 + 4 例阴性，补"台账/状态机 vs 正文/候选"的一致性：人物状态台账（样例 16/20/21，阴性 24）、关系动态（样例 17/22，阴性 25）、伏笔状态机（样例 18/23，阴性 26）、剧情线线程债务（样例 19，阴性 27）。**阳性检测现状均为待落地**（阶段 3 长线治理主体）：样例 16/17/20/21/22 需 L1 扩展或 L2 语义比对，样例 18/19/23 是纯确定性债务检查（机制已建模、检查待落地）；**阴性样例即误报边界，检测落地后必须 0 误报**。登记为评测集基线，检测落地后按同一协议跑检出率 / 误报率。
...
- **检测现状**：待落地——realm/alive 已有 L1 确定性（样例 1/3）；位置 / 伤势等字段的一致性需正文-台账语义匹配，属 L2。
...
- **检测现状**：待落地——persist 已消费 `relation_change` 候选落 Relation 行（nodes.py §7.8）；"正文无变更却相反"的比对是 L1 扩展（L1 现只处理 character_state kind）/ L2 兜底。
...
- **预期检出**：L1 债务（planted 且 `last_touched` 距当前章 > 阈值 → 伏笔烂尾告警）
- **预期 Finding**：`conflict_type=foreshadow, severity=hint, scope=local`
- **检测现状**：待落地——伏笔状态机（planted/developing/resolved/dropped）已建模，persist 落伏笔；"长期无触碰"的债务检查属阶段 3 长线治理主体。
...
- **预期检出**：L1 债务（active 且 `open_duration` > 阈值 → 停滞告警）
- **预期 Finding**：`conflict_type=plotline, severity=hint, scope=local`
- **检测现状**：待落地——`open_duration` 字段已建模，persist 在推进时更新 `last_progress_chapter`（nodes.py §7.9）；"超阈值告警"的债务检查是阶段 3 长线治理主体明确项。样例 13 大纲偏差已 L1 落地，线程停滞是它的"跨章积压"形态。
...
- **检测现状**：待落地——`goal` 字段已建模（CHARACTER_STATE_FIELDS）；语义比对属 L1 扩展 / L2。
...
- **预期 Finding**：`conflict_type=character_state, severity=major, scope=local`
- **检测现状**：待落地——`identity` 字段已建模；与样例 9 区分：样例 9 是 `faction` 阵营归属矛盾，本条直接测台账身份字段一致性（L1 现只处理 character_state 的 realm/alive）。
...
- **预期 Finding**：`conflict_type=relation, severity=major, scope=structural`
- **检测现状**：待落地——与样例 17 同类机制，覆盖不同关系类型（master_student）与方向（→hostile）；persist 已消费 `relation_change`（nodes.py §7.8）。
...
- **预期检出**：L1 债务（developing 且 `last_touched` 距当前章 > 阈值且触发条件已成熟 → 烂尾告警）
- **预期 Finding**：`conflict_type=foreshadow, severity=hint, scope=local`
- **检测现状**：待落地——样例 18 是 `planted` 形态，本条是 `developing` 形态（捡起一半的伏笔长期不落地）；债务检查同属阶段 3。
- **误报控制**：只对"触发条件已成熟却仍不落地"告警（条件在 `trigger`）；未到条件的 developing 不算烂尾（样例 26 阴性）；标 hint，作者决策（收 / 弃）。
- **度量指标**：伏笔烂尾检出率、开放伏笔年龄分布（§16 伏笔回收率）。
...
- **误报控制**：只查"无变更记录却表现相反"；有 `relation_change` 即合法。
...
- **误报控制**：与样例 18 区分：只对"低优先级且长期无触碰"告警；高优先级 + 有条件的长沉合法。
...
- **误报控制**：支线（side）阈值放宽或免检；主线（main）才告警。
...
1. **跑测**：阳性（样例 1–23）埋入章节样本或独立跑样例，跑 L1 + L2，统计**检出率**；阴性（样例 24–32）单独跑正常章节，统计**误报率**（阴性被检出 = 误报 +1）。两组缺一，指标不成立；
2. **调阈值**：L1 阈值宁缺毋滥（plan.md §8.8），先保证阴性 0 误报，再抬阳性检出率；L2 低置信度标 hint 不耗修订预算；
3. **长线**：样例 11–15、18/19、23、26/27、31/32 走周期审计（每 K 章一次），不并入每章点检——避免成本与误报不可控；
