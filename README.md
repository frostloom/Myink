...
- 预留 MCP 标准协议接入外部榜单工具（起点 DaoSearch / 番茄榜单），榜单数据仅作灵感参考注入规划节点、不进入记忆 / 事实层
...
- Python：389 项测试（含 5 项预期失败标记），覆盖编排、校验、记忆、成本闸门、越权矩阵
- Go：网关契约与转发测试全绿
- 前端：31 项单测 + TypeScript 严格构建 + Vite 生产构建
- 契约：35 个端点 OpenAPI 契约回归闸，`aiink contract export && git diff --exit-code` 防漂移
