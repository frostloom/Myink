"""身份断言（plan.md §14.1 ③）：JWT 签发 + 归属校验依赖，替换阶段 2 X-AiInk-User 占位。
...
- `current_user`：读 `X-AiInk-User` 头（网关验证 JWT 后透传的可信身份），供应用层过滤。
...
信任链（§17.2）：网关是唯一公网入口，验 JWT → 解出可信 user_id → 透传 X-AiInk-User →
...
def current_user(x_aiink_user: str | None = Header(None, alias="X-AiInk-User")) -> str | None:
    """读网关透传的已验证身份（X-AiInk-User = JWT 的 sub）。空/缺失 → None（调用方 fail closed）。"""
    return x_aiink_user
