...
def fit_prompt(context: dict, render: Callable[[dict], list[dict]], budget: int) -> list[dict]:
    ...
    for key in ('mid_term_events', 'reflexions', 'plot_threads', 'open_foreshadows', 'recent_openings', 'entity_snapshots'):
        required[key] = []
    ...
    for key in ('long_term_facts', 'mid_term_events', 'reflexions',
                'plot_threads', 'open_foreshadows', 'recent_openings', 'entity_snapshots'):
